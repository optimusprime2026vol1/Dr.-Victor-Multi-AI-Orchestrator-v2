...
Victor production supervision defaults to 15m; minimum recovery cadence is 2m; approved ladder only 15 → 10 → 5 → 3 → 2 minutes. Department-internal heartbeats remain independently contracted.

Priority score: Founder/Victor priority up to25; objective/business impact up to25; urgency up to15; dependency criticality up to15; incident severity up to15; cost/rate-limit/concurrency penalty subtract up to15. The production ladder is bounded to 15/10/5/3/2 minutes.

AI may provide context, but cadence selection is deterministic/policy-bounded. Runtime overrides may accelerate from 15m to 10/5/3/2m; PAUSED keeps diagnostic liveness while business actions stay off.
...
Hard invariants: PRODUCTION DEFAULT=15m; MINIMUM=2m; ONLY APPROVED LADDER; PRIORITY NEVER GRANTS AUTHORITY; DIRECT FOUNDER/AUTHORIZED VICTOR COMMAND=IMMEDIATE EVENT WAKE; NO FRESH VERIFIED LIVENESS=NO ASSUMPTION CAPABILITY LIVE; HEARTBEAT WAKES, VALID CONSTITUTIONAL/AUTHORITY/HEALTH/EVIDENCE STATE AUTHORIZES.

Production runtime ownership is singular: the Cloudflare Victor Telegram Worker is the control plane; department repository workflows are the execution/evidence plane; the Python orchestrator is the policy library, test harness and backup reconciler, not a second production controller.
