...
def folder_id(svc):
    ...
    r = svc.files().list(q=q, fields='files(id,name)', spaces='drive').execute().get('files',[])
    if not r: raise RuntimeError(f'Shared Drive folder not found: {FOLDER_NAME}')
    return r[0]['id']
...
def upsert(svc, fid, path):
    ...
    if found:
        svc.files().update(fileId=found[0]['id'], media_body=media).execute(); return 'updated'
    svc.files().create(body={'name':path.name,'parents':[fid]}, media_body=media, fields='id').execute(); return 'created'

def main():
    svc=service(); fid=folder_id(svc); results={}
    for name in FILES:
        p=MEM/name
        if p.exists(): results[name]=upsert(svc,fid,p)
